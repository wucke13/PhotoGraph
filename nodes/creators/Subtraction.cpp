...
struct SubtractionHandler {
  ...
  static inline handler_out_t perform(ImageGraph::OutNode* in1, ImageGraph::OutNode* in2, bool dither) {
    ...
    return std::make_unique<ImageGraph::nodes::SubtractionNode<T1, T2, T3>>(*out1, *out2, dither);
  }
};

handler_out_t SubtractionCreator::createNode(const TwoDitherInfo& info) {
  return TypeListList<default_numbers_t, default_numbers_t, default_numbers_t>::perform<SubtractionHandler,
                                                                                        handler_out_t>(
