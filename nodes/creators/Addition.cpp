...
struct AdditionPerformer {
  ...
  static inline handler_out_t perform(ImageGraph::OutNode* in1, ImageGraph::OutNode* in2, bool dither) {
    ...
    return std::make_unique<ImageGraph::nodes::AdditionNode<T1, T2, T3>>(*out1, *out2, dither);
  }
};

handler_out_t AdditionCreator::createNode(const TwoDitherInfo& info) {
  return TypeListList<default_numbers_t, default_numbers_t, default_numbers_t>::perform<AdditionPerformer,
                                                                                        handler_out_t>(
